...
# Set environment variable for Windows to use the right backend
import os
...
# Set up the display mode with appropriate flags
...
class DialogueSystem:
    def __init__(self):
        ...
        # Speech integration
        self.speech = SpeechIntegration()
        self.voice_active = False
        self.speech_thread = None
        self.speech_processing = False
        self.voice_config = {
            "HR": "nova",    # Female voice for HR
            "CEO": "onyx"    # Male voice for CEO
        }
    ...
    def start_conversation(self, npc_role="HR", player_pos=None):
        ...
        # Set voice for current NPC
        if npc_role in self.voice_config:
            self.speech.set_voice(self.voice_config[npc_role])
    ...
    def send_message(self):
        ...
        try:
            ...
            if self.voice_active and self.speech.is_speaking:
                self.speech.interrupt_speech()
            ...
            if self.voice_active:
                self.speech.speak_text(ai_message)
    ...
    def render(self):
        ...
        if self.active:
            ...
            # Voice toggle instruction
            voice_status = "ON" if self.speech_processing else "OFF"
            voice_color = (0, 255, 0) if self.speech_processing else (255, 0, 0)
            voice_text_surface = self.font.render(f"Voice Input: {voice_status} (Shift+V to toggle)", True, voice_color)
            self.ui_surface.blit(voice_text_surface, (WINDOW_WIDTH - 300, box_y + 10))
            
            # Speech status indicators
            if self.speech_processing:
                if self.speech.is_speaking:
                    status_text = "NPC Speaking..."
                    status_color = (255, 255, 0)  # Yellow
                elif not self.speech.audio_queue.empty():
                    status_text = "Listening..."
                    status_color = (0, 255, 0)  # Green
                else:
                    status_text = "Ready for voice input"
                    status_color = (0, 200, 255)  # Light blue
                
                status_surface = self.font.render(status_text, True, status_color)
                self.ui_surface.blit(status_surface, (WINDOW_WIDTH - 300, box_y + 35))
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # Check for Shift+V to toggle voice input
            if keys[pygame.K_LSHIFT] and event.key == pygame.K_v:
                if self.speech_processing:
                    self.speech_processing = False
                    self.speech.stop_listening()
                    print("[DialogueSystem] Voice input disabled")
                else:
                    self.speech_processing = True
                    self.speech.start_listening()
                    print("[DialogueSystem] Voice input enabled")
                return
    ...
    def _speech_worker(self):
        """Worker thread to handle speech processing"""
        print("[DialogueSystem] Speech worker started")
        while self.voice_active:
            # Check for speech input if processing is enabled
            if self.speech_processing:
                speech_text = self.speech.get_speech_text()
                if speech_text:
                    print(f"[DialogueSystem] Speech input: {speech_text}")
            ...
            if self.speech_processing and self.speech.is_speaking:
                # If we detect new speech while speaking, interrupt
                if not self.speech.audio_queue.empty():
                    print("[DialogueSystem] Detected interruption")
                    self.speech.interrupt_speech()
            
            time.sleep(0.1)
        
        print("[DialogueSystem] Speech worker stopped")
...
class Game3D:
    ...
    def cleanup(self):
        ...
        print("[Game3D] Cleaning up resources...")
        
        # Clean up dialogue system resources
        if hasattr(self.dialogue, 'speech'):
            self.dialogue.speech.cleanup()
            
        # Clean up OpenGL resources
        if hasattr(self.dialogue, 'ui_texture'):
            glDeleteTextures([self.dialogue.ui_texture])
            
        print("[Game3D] Cleanup complete")
